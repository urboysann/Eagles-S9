...
export default function TekkenDetail() {
  const pathname = usePathname();
  const [timeLeft, setTimeLeft] = useState({ days: 25, hours: 3, minutes: 55, seconds: 19 });
  
  const GOOGLE_FORM_LINK = "https://bit.ly/Pendaftaran-Lomba-PNB-EAGLES-S9";
  const GUIDEBOOK_PDF_PATH = "/files/guidebook-tekken8.pdf";

  // Countdown logic mengikuti data di image_8fef79.jpg
  useEffect(() => {
  ...
  const infoLomba = [
    ...
    { icon: <Users size={22} />, label: "KUOTA", value: "32 PLAYER", sub: "1 VS 1 (SINGLE ELIMINATION)" },
  ];
            ...
            >
              <div>
                <div className="flex items-center gap-3 mb-6">
                  {/* Badge warna Magenta sesuai request */}
                  ...
                  <span className="text-gray-500 text-[10px] font-bold tracking-widest uppercase font-montserrat">NEXT-GEN BATTLE</span>
                </div>
                
                <h1 className="text-6xl md:text-7xl font-black italic leading-[0.85] uppercase font-outfit tracking-tighter">
                  TURNAMEN <br />
                  <span className="text-magenta drop-shadow-[0_0_20px_rgba(255,80,193,0.4)]">TEKKEN 8</span>
                </h1>
              </div>
              ...
              <p className="text-gray-400 text-lg leading-relaxed max-w-xl text-justify font-medium">
                Siapkan kombo terbaikmu di <span className="text-white italic font-bold">PNB EAGLES Season 9.</span> Hadapi petarung tangguh lainnya dalam arena Tekken 8 dan buktikan siapa yang layak menjadi <span className="text-magenta font-bold italic">King of Iron Fist!</span>
              </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-5 bg-white/5 border border-white/10 rounded-[1.5rem] group hover:border-magenta/40 transition-all duration-300">
                  <Swords className="text-magenta mb-3 group-hover:scale-110 transition-transform" size={22} />
                ...
                <div className="p-5 bg-white/5 border border-white/10 rounded-[1.5rem] group hover:border-magenta/40 transition-all duration-300">
                  <Zap className="text-magenta mb-3 group-hover:scale-110 transition-transform" size={22} />
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Platform</p>
                  <p className="text-xl font-black italic uppercase text-white">PlayStation 5</p>
                </div>
              </div>
